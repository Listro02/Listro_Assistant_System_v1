...
2. **Level 3 확인**: 사전 하드코딩된 Level 3 테마 벡터들과 코사인 유사도 계산을 통해 **상위 최대 3개**의 테마 추출
3. **필터링 및 확장**: 
   - 추출된 상위 3개 중 임계치(threshold)를 넘는 테마만 선별
   - 선별된 테마들에 **`T_10`(사용자 프로필 고정 테마)**와 **`""`(테마 미분류/없는 응답)**을 검색 필터에 강제 추가하여 누락 방지

---

#### Phase 2: 국소 검색 (Local Retrieval — L2 주검색 + L1 연관 탐색)

1. **Pre-filter**: Phase 1에서 조합된 테마 ID 배열(선별 테마 + `T_10` + `""`)을 메타데이터 조건으로 적용 → 검색 공간 설정
...
#### Phase 1: 실시간 로깅 및 트리거
- **목적**: 데이터 유실을 방지하고 메인 대화 응답의 지연을 막는 진입점
- **동작**:
  1. 단기 윈도우에서 20턴을 초과해 밀려난 대화 뭉치가 발생하면 통합 파이프라인 트리거
  2. 밀려난 대화 뭉치와 오버랩을 위한 추가 잔존 2턴을 결합하여 처리 구간(Target Range) 설정
  3. 처리 구간 정보를 불변 렛저(Level 0)와 연결된 WAL(Write-Ahead Log) 큐에 등록 후 즉시 메인 루프 반환 (이후 과정은 백그라운드 비동기 처리)

#### Phase 2: 구조화된 순차 추출 (2-Pass)
- **목적**: 원시 대화에서 객관적 사실(Level 2)과 에피소드 맥락(Level 1)을 분리하여 정제
- **동작**:
  1. **Pass 1 (사실 추출)**: 해당 구간의 대화에서 특징적인 숫자, 설정값, 도메인 지식 등 영구 보존 가치가 있는 독립적인 객관적 사실들만 추출하여 고유 ID(예: `F_101`) 발급
  2. **Pass 2 (맥락 생성)**: 원본 텍스트와 Pass 1에서 추출된 사실들을 함께 병합 분석하여, 대화의 전반적인 다차원 맥락(핵심 주제, 기술 스택, 결론 등)을 생성하고 에피소드 ID(예: `E_42`) 발급

#### Phase 3 및 3.5: 임베딩 및 충돌 감지 (계층 내 매핑)
- **목적**: 추출된 텍스트 데이터를 검색 가능한 벡터 공간으로 변환하고, 기존 기억과의 무결성을 검증
- **동작**:
  1. **임베딩**: Pass 2에서 생성된 Level 1 요약문과 Pass 1의 Level 2 사실들을 각각 벡터화
  2. **유사도 기반 충돌 검증**: 새로 추출된 Level 2 사실 벡터를 기존 DB의 사실들과 비교하여 임계치를 초과하는 충돌 후보군 도출
  3. **상태 전이 (Supersede)**: 충돌 후보군이 발견될 경우, 시간 정보와 원문을 바탕으로 LLM이 논리적 모순이나 정보 갱신 여부를 판별. 기존 정보가 대체되어야 한다고 판단되면 기존 정보의 상태를 `superseded`(대체됨)로 전이하고 새 정보의 ID로 연결

#### Phase 4: 전역 테마 매핑 및 최종 커밋
- **목적**: 생성된 에피소드(L1)와 사실(L2)들을 벡터 공간에 저장하고, 하향식 탐색이 가능한 계층적 메타데이터 구조 완성
- **동작**:
  1. **독립적 테마 매핑 (L2 → L3)**: 에피소드의 테마를 일괄 상속받는 것이 아니라, **각각의 사실 노드(Level 2)가 자신의 벡터를 바탕으로 독립적으로 가장 적합한 전역 테마(Level 3)에 매핑**(`parent_theme_id` 할당)됩니다. (하나의 에피소드 안에서도 여러 주제의 사실이 도출될 수 있으므로 독립성 보장)
  2. **상호 매핑 (L1 ↔ L2)**: 에피소드(L1)는 자신이 배출한 사실들의 ID(`extracted_facts`)를 가지고, 사실(L2)은 자신이 도출된 에피소드의 ID(`source_episode_id`)를 가지어 양방향 참조를 구성합니다.
  3. **계층적 구조 성립 (L3 → L2 → L1)**: 이를 통해 전역 테마(L3)를 검색하여 관련 사실(L2)을 찾고, 그 사실을 통해 다시 원본 에피소드 맥락(L1)과 원시 대화(L0)로 파고들 수 있는 완벽한 하향식 탐색 트리가 완성됩니다.
  4. **최종 커밋**: Level 1, Level 2 벡터 데이터베이스에 최종 저장하고, 처리가 완료된 WAL 큐의 상태를 완료(Complete)로 업데이트하여 트랜잭션 종료
