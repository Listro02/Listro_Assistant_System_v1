...
## ⑥ 성능 검증 (V1 vs V2 벤치마크 결과)

실제 구현체를 대상으로 진행한 벤치마크 테스트 결과, xMemory(V2) 아키텍처는 기존 JSONL 기반(V1) 시스템 대비 압도적인 성능 개선을 달성했습니다.

| 메트릭 | V1 (Legacy) 평균 | V2 (xMemory) 평균 | 변화율 |
|---|---:|---:|---:|
| **응답 지연 시간** | 7.17초 | 3.19초 | **-55.5%** |
| **API 호출 횟수** | 5.0회 | 3.0회 | **-40.0%** |
| **평균 토큰 소모** | 7,044 tok | 382 tok | **-94.6%** |

**주요 성능 개선 요인:**
1. **프롬프트 최적화**: V1은 8턴 이상의 전체 대화 내역을 일괄 주입하여 응답 생성(`talking`)에만 약 2,700토큰을 소모했습니다. V2는 벡터 DB를 통해 **관련된 Level 2 사실만 선별 주입**하여 `talking` 단계의 프롬프트 토큰을 **100토큰 내외**로 급감시켰습니다.
2. **적응형 파이프라인**: V1이 모든 발화에 대해 5회의 API 동기 호출(추출, 판단, 생성, 요약 등)을 강제하는 반면, V2는 RAG가 불필요한 일상 대화(`NO_RAG`) 시 단 2회 호출만으로 1초 대에 빠르게 응답을 반환합니다.
3. **통합 비용의 비동기화 분리**: V1은 사용자가 답변을 기다리는 응답 파이프라인 상에 요약 및 사실 추출 로직(`make_memory`)이 포함되어 체감 지연이 극심했습니다. V2는 이를 비동기 백그라운드 파이프라인으로 완전히 분리하여 체감 성능을 혁신했습니다.

상세한 시나리오별 분석과 측정 데이터는 [벤치마크 분석 리포트](benchmark_results.md)에서 확인할 수 있습니다.
