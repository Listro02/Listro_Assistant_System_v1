...
기존 L.A.S.의 메모리 시스템은 JSONL 기반의 3계층(단기, 중기 요약, 장기 사실) 아키텍처로, 외부 DB 없이 가볍게 동작하는 장점이 있었습니다. 하지만 대화가 누적됨에 따라 **기억 로드로 인한 응답 지연(Latency)**, **장기 기억의 비가역적 오염(Memory Poisoning)**, 그리고 **토큰 폭발(Token Explosion)** 등의 한계가 확인되었습니다.
...
## 📊 메모리 시각화 (Knowledge Graph)

새로운 메모리 시스템은 메타데이터 형태로 저장하여 계층화했기에 그동안의 대화 내역을 구현했던 시스템으로 지식화 했습니다. 다음은 이를 지식 그래프 형태로 시각화한 것입니다.

- **`memory_graph.html`**: [지식 그래프 시각화 보기](https://listro02.github.io/Listro_Assistant_System_v1/LAS_Memory_V2/memory_graph.html)

---

## ⚙️ 실행 방법 (Getting Started)
- 없음.
- 본 프로젝트는 기본적으로 L.A.S. v1 구조에서 메모리 시스템을 xMemory로 교체한 것입니다. 따라서 L.A.S. v1과 실행 방법이 동일한데, 문제는 해당 시스템이 하드코딩된 부분이 많아 외부 환경에서 실행을 장담할 수 없다는 것입니다.
- 결론적으로, xMemory Test를 원하시는 분은 본 프로젝트의 담당자(Listro)에게 연락주시길 바랍니다.
...
- 기존의 L.A.S. kivy UI에 xMemory 활성화 버튼을 두어, 새로운 메모리 아키텍처를 바탕으로 상호작용할 수 있도록 구현해두었습니다.
